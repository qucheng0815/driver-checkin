...
function loadCurrent() {
  ...
  .then(function(result) {
    ...
    tbody.innerHTML = result.data.map(function(r) {
      return '<tr><td><strong>' + r.plate + '</strong></td><td>' + r.name + '</td><td>' + r.company + '</td><td>' + r.checkin_time + '</td></tr>';
    }).join('');
  })
...
function loadHistory(page) {
  ...
  .then(function(result) {
    ...
    tbody.innerHTML = result.data.map(function(r) {
      return '<tr><td><strong>' + r.plate + '</strong></td><td>' + r.name + '</td><td>' + r.company + '</td><td>' + r.phone + '</td><td>' + r.checkin_time + '</td><td>' + (r.checkout_time || '-') + '</td><td class="' + (r.checkout_time ? 'status-out' : 'status-in') + '">' + (r.checkout_time ? '已离场' : '在场中') + '</td></tr>';
    }).join('');
...
function changePage(delta) { loadHistory(currentPage + delta); }
function clearFilter() { document.getElementById('filterDate').value = ''; loadHistory(1); }
